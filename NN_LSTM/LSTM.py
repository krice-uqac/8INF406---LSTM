...
class LSTMTrainer:
    ...
    def train(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            self.model.eval()
            with torch.no_grad():
                with tqdm(total=len(self.val_loader), desc=f"Epoch {epoch+1}/{num_epochs}, Train Loss: {train_loss/len(self.train_loader):.4f}, Val Loss: {val_loss/len(self.val_loader):.4f}") as pbar:
